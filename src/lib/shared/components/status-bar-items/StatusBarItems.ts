...
export class StatusBarItem {
  ...
  public serialize(): TSerializableStatusBarItem {
    if (!this.defaultValue.label) throw new Error(`Label not defined for "${this.key}" status bar item`);
    if (!this.defaultValue.side) throw new Error(`Side not defined for "${this.key}" status bar item`);
